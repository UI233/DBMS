...
本系统由$Interpreter$模块、$Catalog\ Manager$模块、$Index\ Manager$模块、$Record\ Manager$模块、$Buffer\ Manager$模块、$API$模块、$DB\ Files$模块。
...
| 删除记录 | 无该表；<br/>表中无条件中提出的属性；<br>
条件中属性的类型和传入值的类型不匹配； |
| 建立索引 | 索引已存在；<br>该索引的属性名不存在；<br>                   |
| 删除索引 | 该索引不存在；<br>                                           |
| 删除表   | 无此表；<br>                                                 |

根据以上语句的对应可能错误进行检查，调用$CatalogManager$的对应接口获得表、索引、属性的信息，若发生错误则抛出对应错误，由$Interpreter$ $Catch$错误并输出信息。
...
2. $IM::BPTree$
   ...
   此类每次实例化都是为了一次对于B+树的操作，为了实现这些操作，我们还实现了下列对外的接口函数：
...
$Index$文件基本有两个部分组成，分别是$Index~Header$,和$Index~Data$
...
<img src="./img/test4-1.png" style="zoom:70%" />

##### case2

<img src="./img/test4-2-1.png" style="zoom:70%" />

<img src="./img/test4-2-2.png" style="zoom:70%" />

<img src="./img/test4-2-3.png" style="zoom:70%" />
...
<img src="./img/test6-1-2.png" style="zoom:70%" />
